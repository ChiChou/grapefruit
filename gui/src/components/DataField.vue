<template>
  <div class="data-field">
    <div v-if="depth == 0 && !isExpandableType">
      <span
        class="value"
        :class="valueClass"
...
function subFieldCount(value) {
  ...
  else if (value && typeof value === 'object') return Object.keys(value).length
  return 0
}
...
export default {
  ...
  computed: {
    ...
    formattedValue() {
      ...
      if (this.fieldOptions.abstract) {
        ...
        return typeMatch ? escape(typeMatch[1]) : `<span>"</span>${escape(value)}<span>"</span>`
      } else if (isNaN(value)) {
        return 'NaN'
  ...
  mounted() {
    if (this.depth === 0) this.expanded = true
  },
  ...
  methods: {
    ...
    toggle() {
      if (this.isExpandableType) this.expanded = !this.expanded
    },
...
.dark .arrow {
  &.up {
    border-bottom-color: lighten($arrow-color, 80);
  }

  &.down {
    border-top-color: lighten($arrow-color, 80);
  }

  &.right {
    border-left-color: lighten($arrow-color, 80);
  }

  &.left {
    border-right-color: lighten($arrow-color, 80);
  }
}
